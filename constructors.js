...
Movie.prototype.changeRating = function(num) {
    return (this.rating + 74) / 2;
}

////////// PROBLEM 5 //////////

// Write a constructor function called User. This function should take in 4 parameters called name, age, email, and
// savedPosts in that order. Name and email will be strings, age will be a number and savedPosts will be an array of objects. 
...
// It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters
// to create a new object and add it to the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
User.prototype.addSavedPost = function(id, title, rating) {
    ...
    this.savedPosts.push(something);
};
...
User.prototype.removeSavedPost = function(postId) {
  const index = this.savedPosts.findIndex(val => val === postId)
...
// The first will be an id (a number) and the second will be the new rating (a number). Use the id to find the matching object in the savedPosts array.
...
User.prototype.changePostRating = function(id, rating) {
 const match = this.savedPosts.findIndex(val => val.id === id);
 return this.savedPosts[match].rating = rating;
}
